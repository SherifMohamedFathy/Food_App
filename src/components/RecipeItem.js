  ...
  return (
    <>
      {loading && data ? (
        ...
        <>
          <div className="recipe-item container  px-12 mx-auto py-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
            <div className="left row-start-2 lg:row-start-auto ">
              <div className="singleItemImage h-96 overflow-hidden rounded-xl group  ">
                <img
                  src={data.image_url}
                  className=" w-96 h-96 md:w-full md:h-full rounded-xl  object-cover block group-hover:scale-105 duration-300"
                  alt=""
                />
              </div>
              <div className="ingredients  mt-4 ">
                <span className="text-2xl font-semibold text-red-500 ">Ingredients:</span>
                <ul className="flex flex-col gap-3">
                  {data?.ingredients?.map((ingredient, i) => (
                    <li key={i}>
                      <span className="font-bold text-2xl text-red-300 ">✓ </span>
            ...
            <div className="right flex flex-col gap-3">
              <span className="recipe-publisher text-sm text-cyan-500 font-medium">{data.publisher}</span>
              <h2 className="recipe-title text-4xl truncate font-semibold text-red-500">{data.title}</h2>
              <div className="serving_cooking-time flex gap-5 uppercase tracking-widest font-semibold text-rose-300">
                ...
                <div className="cooking-time">
                  Cooking Time:
                  {data?.cooking_time < 60 ? String(data.cooking_time) + "min" : durationCalc(data?.cooking_time / 60)}
                </div>
              </div>
              ...
              <div className="btns flex gap-5">
                ...
                >
                  {itemSaveStatus ? "- Remove from favourites" : "+ Save as favourite"}
                </button>
